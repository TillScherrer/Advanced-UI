...
namespace OculusSampleFramework
{
	...
	public class InteractableToolsCreator : MonoBehaviour
	{
		...
		[HideInInspector] public int isSettenUp = -1;
		//InteractableTool[] SettenUpLHT = new InteractableTool[10];
		//public InteractableTool[] GetHandTools(bool left) { return left ? SettenUpLHT : SettenUpRHT; }
		//InteractableTool[] SettenUpRHT = new InteractableTool[10];
		private int leftHandAssignments = 0;
		private int rightHandAssignments = 0;
		...
		private void AttachToolToHandTransform(Transform tool, bool isRightHanded)
		{
			...
			toolComp.Initialize();
            //vvv RETARGETING CUSTOM ADDITION vvv
            /*if (isRightHanded)
            {
				Debug.Log("want to assign on right hand with index: " + rightHandAssignments);
				SettenUpRHT[rightHandAssignments] = newTool.GetComponent<InteractableTool>();
				rightHandAssignments++;
			}
            else
            {
				Debug.Log("want to assign on left hand with index: " + leftHandAssignments);
				SettenUpLHT[leftHandAssignments] = newTool.GetComponent<InteractableTool>();
				leftHandAssignments++;
			}*/
		}
	}
